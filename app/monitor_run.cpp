...
int main(int argc, char *argv[]) {
  ...
  std::string pipe_nominal;
  if (argc != 9) {
    std::cout << "Uso: ./monitor -b tam_buffer -t file_temp -h file_ph -p "
                 "pipe_nominal\n"
                 "Donde:\n"
                 "tam_buffer: el tamaño del buffer de datos\n"
                 "file_temp: el nombre del archivo de temperatura\n"
                 "file_ph: nombre del archivo de ph\n"
    ...
    Monitor monitor_sensores(tam_buffer);
    monitor_sensores.run(file_temp,file_ph,pipe_nominal);
